...
### 8.4 Documentation site — docs.chatrealty.io + clipboard LLM prompts (NEW — Agents 33–35)

The public developer-docs site and the most differentiated "made for Claude" surface — §3.9 made concrete. **One source of truth feeds three consumers:** the **docs site** (humans read), the **MCP `guide://` build-guide** (Claude reads in-loop), and **`llms.txt`** (any LLM ingests). They cannot drift because they share the same MDX/prompt source, and the API reference is generated from the schema.

**Framework — recommended: Fumadocs** (Next.js-native MDX docs — matches the stack, built-in search, first-class OpenAPI reference + `llms.txt`, full control for custom components). Fast-path alternative: **Mintlify** (hosted, gorgeous, built-in "Copy for LLM" + AI assistant) if hosted-and-beautiful beats full control. Content (MDX) is portable either way; decide before Agent 33.

**Signature feature — clipboard LLM prompts.** Every setup step ships a curated, copy-paste **`<ClaudePrompt>`** ("Copy for Claude" + an "Open in Claude" deep link) that drives that step: connect your MLS feed, seed your DB, scaffold a listings page, add the map, wire favorites + lead capture, build neighborhoods. The customer pastes it into their own Claude (BYOK) and it builds. **The same prompt set IS the MCP build-guide** (`guide://chatrealty/*`) — one source, so the human-facing and Claude-facing guides are identical.

**Content set (robust):** Quickstart (signup → token → connect MLS → seed → first query); Concepts (tenancy, BYOD, BYOK, the RESO Data Dictionary, custom fields, the **attribution rule**, neighborhoods, lead capture); API Reference (OData endpoints, auth, scopes, errors — generated from OpenAPI); MCP (connect, the two tiers, tools); Guides/recipes (each with a `<ClaudePrompt>`); the customer-side sync setup. Plus `llms.txt` / `llms-full.txt` and "copy page as markdown" on every page.

- **Agent 33 — Docs site scaffold + deploy.** OWNS `apps/docs/*` (Fumadocs app), the `docs` subdomain reservation (add `docs` to the reserved list in `src/proxy.ts` like `agent`, so it never resolves as a tenant; point Vercel at the docs app via the domain registry), nav/search/theme. Depends: domain registry (existing).
- **Agent 34 — `<ClaudePrompt>` component + the prompt library (shared with the MCP build-guide).** OWNS `apps/docs/components/claude-prompt.tsx`, `src/lib/build-guide/prompts/*` — the single prompt source consumed by BOTH the docs site and the MCP `guide://` resources. Depends: 33, MCP build-guide (Agents 07/31). Accept: a given prompt renders on the docs site (copyable) AND resolves as the matching `guide://` MCP resource — one source, verified byte-identical.
- **Agent 35 — Content + OpenAPI reference + llms.txt.** OWNS `apps/docs/content/**/*.mdx`, the OpenAPI spec generator (from the OData routes), the `llms.txt`/`llms-full.txt` generator + "copy as markdown". Depends: 33, the API routes (Spec 6), the Data Dictionary (Agent 03). Accept: the API reference is **generated** (not hand-written) and matches the live routes; every concept page restates the attribution rule; `llms.txt` enumerates all pages.

**Sequencing:** content scaffolding + the prompt library can start in **Phase 0** (no keystone needed); the OpenAPI reference integrates once the API routes are live (Phase 2+). Total now **~35 agent-tasks**.
