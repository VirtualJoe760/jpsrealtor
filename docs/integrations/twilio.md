...
| **2. A2P onboarding + per-agent legal** ⏳ in progress | **Clause generators drafted** (`src/lib/messaging/sms-legal.ts`: `smsTermsClause`/`smsPrivacyClause`) + see `sms-compliance.md`. Remaining (external/user): ISV registration, per-agent Brand+Campaign from onboarding business info (`User.messaging.a2p`), and publishing the clauses on each agent's Terms/Privacy pages as opt-in evidence. |
