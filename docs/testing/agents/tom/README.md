...
| `TOOLS.md` | Environment facts — API endpoints, credentials sources, how to dispatch Test Claude, browser judging setup | The tooling or environment changes (new API path, new credential source, spawn arguments change) |
...
- Steps are numbered for a reason — step 0 (mid-session check) must run before
  step 1 (poll), which must run before step 2 (brief). Don't reorder.
...
2. Database connector (from chatrealty.io setup wizard)
3. Site token (from chatrealty.io Integrations)

The judge token (`CHATREALTY_JUDGE_TOKEN`) is for the testing API only. If you
see it being used for anything else in these files, that is a bug.
...
3. If it changes something Tom needs to know immediately, tell Joe to trigger
