...
| 2026-06-05 | _(pending)_ | PWA manifest/SW fixes: `manifest-v2.json` had white `theme_color`/`background_color` on a black-default app (white splash flash) → set both to `#000000`; removed `screenshots` entries that pointed at missing files (`/screenshots/home.png`, `/screenshots/mobile-home.png` — 404 in install UI). `sw.js` push notifications referenced a missing `/icons/badge-72x72.png` → repointed to `/icons/icon-72x72.png` and bumped `SW_VERSION` 1.0.1→1.0.2. **Still open:** manifest name/short_name/description are hardcoded to Joseph Sardella (multi-tenant domains show his branding — needs a dynamic per-owner manifest route); SW does no app-shell precaching (`CACHE_NAME` is dead code — no offline support). |
