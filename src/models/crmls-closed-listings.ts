// src/models/crmls-closed-listings.ts
...
// -----------------------------
// CRMLS Closed Listing Interface
// -----------------------------

export interface ICRMLSClosedListing extends Document {
  listingId: string;
  slug: string;
  slugAddress?: string;
...
// -----------------------------
// CRMLS Closed Listing Schema
// -----------------------------

const CRMLSClosedListingSchema = new Schema<ICRMLSClosedListing>({
...
CRMLSClosedListingSchema.index({ latitude: 1, longitude: 1, closeDate: 1, propertyType: 1 });

// Price range queries for comps
CRMLSClosedListingSchema.index({ closePrice: 1, closeDate: 1 });

// Beds/Baths filtering for comps
CRMLSClosedListingSchema.index({ bedroomsTotal: 1, bathroomsFull: 1, closeDate: 1 });

// City-based searches
CRMLSClosedListingSchema.index({ city: 1, closeDate: 1, closePrice: 1 });

// Subdivision searches for comps
CRMLSClosedListingSchema.index({ subdivisionName: 1, closeDate: 1 }, { sparse: true });

// Square footage + close date
CRMLSClosedListingSchema.index({ livingArea: 1, closeDate: 1 });

// -----------------------------
// Model Export
// -----------------------------

export const CRMLSClosedListing: Model<ICRMLSClosedListing> =
  mongoose.models.CRMLSClosedListing || mongoose.model<ICRMLSClosedListing>("CRMLSClosedListing", CRMLSClosedListingSchema);
