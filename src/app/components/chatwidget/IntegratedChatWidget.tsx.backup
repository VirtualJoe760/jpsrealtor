  ...
  const handleSend = async (userMessage: string) => {
    ...
    while (currentAttempt <= MAX_RETRIES) {
      try {
        ...
        // NOTE: This IntegratedChatWidget uses old function calling system (deprecated)
        // New chat system uses /api/chat/stream with matchLocation and searchCity tools
...
- Keep responses conversational and helpful`;
    ...
    try {
      ...
      if (functionCall && functionCall.type === "matchLocation") {
        ...
        logChatMessageAsync(
          "system",
          `Function call detected: searchListings (DEPRECATED)`,
          userId,
          {
            functionCall: functionCall.params,
            timestamp: new Date().toISOString(),
            note: "searchListings is deprecated - use /api/chat/stream with matchLocation or searchCity"
          }
        );
          ...
          <div className="flex-1 relative overflow-hidden">
            ...
            {displayMessages.map((message, index) => {
              ...
              if (message.role === "assistant" && (parsed.carousel || parsed.map)) {
                console.log("[INTEGRATED-CHAT] Parsed components:", {
