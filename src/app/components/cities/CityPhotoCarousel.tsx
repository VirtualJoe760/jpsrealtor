...
interface CityPhotoCarouselProps {
  cityId: string;
  cityName: string;
  limit?: number;
}

export default function CityPhotoCarousel({
  cityId,
  cityName,
  limit = 20,
}: CityPhotoCarouselProps) {
  ...
  const fetchPhotos = async () => {
    ...
    try {
      const response = await fetch(
        `/api/cities/${cityId}/photos?limit=${limit}`
      );
    ...
    <div className="space-y-4">
      ...
      <div className="relative w-full h-96 bg-gray-900 rounded-lg overflow-hidden group">
        <Image
          ...
          alt={currentPhoto.caption || `${cityName} - Photo ${currentIndex + 1}`}
          fill
