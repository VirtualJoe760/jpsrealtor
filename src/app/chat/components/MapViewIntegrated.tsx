...
export default function MapViewIntegrated() {
  const {
    visibleListings,
    selectedListing,
    selectedFullListing,
  ...
  const { isSidebarCollapsed } = useEnhancedChat();
  ...
  useEffect(() => {
    setMounted(true);
    console.log("🗺️ MapViewIntegrated mounted, preloaded:", isPreloaded, "listings:", visibleListings.length);

    // Load listings if not preloaded yet
    if (!isPreloaded && !isLoading) {
      console.log("🚀 Loading MLS listings for map view...");
  ...
  if (!mounted) {
    return (
      <motion.div {...crossDissolve(0.6)} className="h-full w-full relative bg-black">
  ...
  return (
    <motion.div
      ...
      onAnimationComplete={() => {
        // Force map to update after transition
        setTimeout(() => {
          window.dispatchEvent(new Event('resize'));
        }, 50);
      }}
      ...
      {isLoading && visibleListings.length === 0 ? (
        ...
        <>
          ...
          <AnimatePresence>
            {!favoritesPannelOpen && (
              <motion.button
                ...
                className="absolute top-4 right-4 z-40 bg-black/80 backdrop-blur-lg px-4 py-2 rounded-full border border-neutral-700 hover:border-purple-500 transition-all flex items-center gap-2"
              >
          ...
          <motion.button
            ...
            className="absolute bottom-4 left-4 z-40 bg-black/80 backdrop-blur-lg px-4 py-2 rounded-lg border border-neutral-700 hover:border-purple-500 transition-all flex items-center gap-2 group"
            title={
          ...
          >
            {mapStyle === 'toner' ? (
              <>
                <MapIcon className="w-5 h-5 text-neutral-300 group-hover:text-purple-400 transition-colors" />
              ...
              <>
                <Satellite className="w-5 h-5 text-neutral-300 group-hover:text-purple-400 transition-colors" />
                <span className="text-sm text-white font-medium">Satellite</span>
              </>
            ) : (
              <>
                <Globe className="w-5 h-5 text-neutral-300 group-hover:text-purple-400 transition-colors" />
