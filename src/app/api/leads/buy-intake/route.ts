...
export async function POST(request: NextRequest) {
  try {
    ...
    const ipLimit = checkRateLimit(`buy-intake:ip:${ipForLimit}`, { max: 10, windowMs: 60 * 60 * 1000 });
    if (!ipLimit.ok) {
      return NextResponse.json(
        { error: ipLimit.error },
    ...
    if (createAccount) {
      try {
        ...
        if (!user) {
          ...
          const signupOrigin = await resolveSignupOrigin(request, "buy_intake");
    ...
    sendLeadEvent(
      ...
      { content_category: "buy_inquiry", content_name: cityName || "Buy Lead" }
    ).catch(() => {});
