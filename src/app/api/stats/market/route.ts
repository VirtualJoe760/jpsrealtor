 ...
 * - Economic indicators (inflation, unemployment, GDP, etc.)
 *
 * This endpoint is cached for 15 minutes to reduce API calls
 */
...
async function fetchHistoricalRates(): Promise<{ date: string; thirtyYear: number; fifteenYear: number }[]> {
  try {
    ...
    console.error('[market-stats] FRED API error:', error);
    return [];
  }
}
...
async function fetchEconomicIndicators(): Promise<{
  ...
  try {
    ...
    console.error('[market-stats] Economic indicators error:', error);
    return {
      inflation: null,
      unemployment: null,
      homePriceIndex: null,
...
async function fetchMortgageRates(): Promise<MortgageRateData> {
  try {
    ...
    console.error('[market-stats] Mortgage rate fetch error:', error);
