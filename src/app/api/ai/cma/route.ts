  ...
  try {
    ...
    if (comparables.length > 0) {
      console.log('📋 First comparable sample:', {
        ...
        closePrice: (comparables[0] as any).closePrice,
    ...
    const pricesPerSqft = comparables
      .filter(c => c.livingArea && ((c as any).closePrice || c.listPrice))
      .map(c => {
        const price = (c as any).closePrice || c.listPrice || 0;
    ...
    const allPrices = comparables.map(c => (c as any).closePrice || c.listPrice || 0);
    ...
    const response = {
      ...
      comparables: comparables.map(c => ({
        ...
        closePrice: (c as any).closePrice,
        finalPrice: (c as any).closePrice || c.listPrice, // Use close price if available
        pricePerSqft: ((c as any).closePrice || c.listPrice) && c.livingArea
          ? ((c as any).closePrice || c.listPrice) / c.livingArea
          : null,
        beds: c.bedroomsTotal,
        baths: c.bathroomsTotalInteger || c.bathroomsTotalDecimal,
        sqft: c.livingArea,
        lotSize: (c as any).lotSizeSquareFeet,
        daysOnMarket: c.daysOnMarket,
        yearBuilt: c.yearBuilt,
        subdivision: c.subdivisionName,
        status: c.standardStatus || (c as any).mlsStatus,
        mlsSource: (c as any).mlsSource,
        pool: (c as any).poolYN,
        spa: (c as any).spaYN,
        garageSpaces: (c as any).garageSpaces,
        closeDate: (c as any).closeDate,
      })),
      estimatedValue,
      investmentAnalysis,
      marketContext,
