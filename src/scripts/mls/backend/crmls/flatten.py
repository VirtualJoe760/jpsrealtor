...
input_path = project_root / "local-logs" / "crmls" / "all_crmls_listings_with_expansions.json"
output_path = project_root / "local-logs" / "crmls" / "flattened_crmls_listings.json"
...
def run():
    ...
    print(f"📄 Loading CRMLS listings from {input_path}")
    ...
    print(f"\n✅ Flattened {len(flattened)} CRMLS listings to {output_path}")
    if skipped:
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Error in CRMLS flatten.py: {e}")
        exit(1)
