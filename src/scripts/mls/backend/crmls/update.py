...
# CRMLS Listing Status Update Script
...
LOG_DIR = Path(__file__).resolve().parents[5] / "local-logs" / "crmls" / "status-logs"
...
def main():
    ...
    total = len(listings)
    print(f"🔍 Checking Spark status for {total} CRMLS listings ({', '.join(active_statuses)})")
