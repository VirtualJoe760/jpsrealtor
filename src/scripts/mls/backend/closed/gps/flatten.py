...
input_path = project_root / "local-logs" / "closed" / "gps" / "all_gps_closed_listings_with_expansions.json"
output_path = project_root / "local-logs" / "closed" / "gps" / "flattened_gps_closed_listings.json"
...
def run():
    ...
    print(f"📄 Loading GPS closed listings from {input_path}")
    ...
    print(f"\n✅ Flattened {len(flattened)} GPS closed listings to {output_path}")
    if skipped:
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Error in GPS closed flatten.py: {e}")
        exit(1)
