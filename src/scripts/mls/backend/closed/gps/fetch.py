...
LOCAL_LOGS_DIR = Path(__file__).resolve().parents[6] / "local-logs" / "closed" / "gps"
LOCAL_LOGS_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = LOCAL_LOGS_DIR / "all_gps_closed_listings_with_expansions.json"
...
def fetch_all_closed_listings():
    ...
    print(f"🚀 Fetching GPS CLOSED listings with expansions: {', '.join(EXPANSIONS)}")
    ...
    while True:
        # Filter for GPS MLS ID + property types + Closed status (sold properties)
        ...
        for attempt in range(retries):
            try:
                ...
                if res.status_code == 200:
                    ...
                    if not batch:
                        print("✅ No more GPS closed listings to fetch")
                        break
    ...
    if not listings:
        raise Exception("❌ No GPS closed listings fetched")

    try:
        with OUTPUT_FILE.open("w", encoding="utf-8") as f:
            json.dump(listings, f, indent=2)
        print(f"✅ Saved {len(listings)} GPS closed listings to {OUTPUT_FILE}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ Error in GPS closed fetch.py: {e}")
        exit(1)
