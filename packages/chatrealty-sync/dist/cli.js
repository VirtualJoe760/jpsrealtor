...
program
    .command("access")
    .description("Show EVERYTHING your data key can reach — associations, property types and statuses, each with an exact count — so you can choose what to sync before provisioning anything.")
    .action(async () => {
    ...
    try {
        ...
        const mb = (rows) => `${Math.round((rows * KB) / 1024)} MB`;
        if (a.total != null) {
            console.log(`\n  Everything this key can see: ${n(a.total)} listings (~${mb(a.total)})\n`);
        }
        const table = (title, rows, envVar) => {
            if (rows.length === 0) {
        ...
        const forSale = a.statuses
            .filter((s) => FOR_SALE_STATUSES.includes(s.name))
            .reduce((t, s) => t + s.count, 0);
        if (forSale > 0) {
            console.log(`  Default if you choose nothing: for-sale only — ${n(forSale)} listings (~${mb(forSale)}).`);
            console.log(`  That is what your site's browse displays. Closed history is what makes a`);
            console.log(`  feed large; add it with RESO_STATUSES only if you need comps in your own DB.\n`);
        }
    }
    catch (err) {
...
program
    .command("networks")
    .description("List the MLS networks/associations your data key can see, with a sampled share of each. (Prefer `access` — it counts exactly and covers types + statuses too.)")
...
program
    ...
    if (!conn) {
        console.error("[status] CHATREALTY_DB_URL is not set — run `npx @chatrealty/sync init --token crt_live_…` first.");
        process.exitCode = 1;
        return;
    }
    try {
...
program
    ...
    const conn = process.env.CHATREALTY_DB_URL || process.env.NEON_POOLED_CONN_URI || "";
    if (!conn) {
        bad("CHATREALTY_DB_URL is not set — run `npx @chatrealty/sync init --token crt_live_…` first.", "connection");
    }
    else {
        try {
            ...
            if (!t.rows[0]?.ok) {
              ...
              WHERE standard_status = 'Active' GROUP BY 1;`);
                ...
                const ph = await client
                    .query(`SELECT count(*) FILTER (WHERE primary_photo_url IS NOT NULL)::int AS with_photo,
                      count(*)::int AS n
                 FROM property WHERE standard_status = 'Active';`)
                    .catch(() => ({ rows: [{ with_photo: 0, n: 0 }] }));
                ...
                if (activeForSale > 0) {
                    ...
                    bad(`0 Active FOR-SALE listings. You do have ${activeOther} Active row(s) —\n` +
                        ...
                        "    an API failure: keep seeding until for-sale inventory arrives.", "seeding");
                }
                else {
                    bad(`0 Active listings (${total} rows, all non-Active). Your site will show an\n` +
                        ...
                        "    comps, not inventory. This is a SEEDING gap, not an API failure.", "seeding");
                }
                ...
                if (usedBytes > 0) {
                    ...
                    if (pct >= 90) {
                        bad(`database storage is ${pct}% full (${usedMb} MB of about ${STORAGE_LIMIT_MB} MB).\n` +
                            ...
                            "    Your existing listings keep serving. See 'What to do next' at the end.", "storage");
                    }
                    else if (pct >= 70) {
                        warn(`database storage is ${pct}% full (${usedMb} MB of about ${STORAGE_LIMIT_MB} MB).\n` +
                ...
                if (activeRows > 0) {
                    if (withPhoto === 0) {
                        warn("none of your Active listings have a photo. Cards, the map popups and the\n" +
                        ...
                        okMark(`${withPhoto} of ${activeRows} Active listings have a photo` +
                ...
                if (seedInFlight) {
                    bad(`the seed is INCOMPLETE — pass "${st?.pass_mode ?? "seed"}" is still in flight\n` +
                        ...
                        "    or let the deployed hourly Vercel cron finish it over a few ticks.", "seeding");
                }
    ...
    if (failures === 0) {
        ...
        if (failKinds.has("seeding") && !failKinds.has("storage")) {
            console.log("  • Check 1 (database) connected fine — what failed is that your listings aren't all\n" +
        ...
        if ((failKinds.has("connection") || failKinds.has("storage")) && feedFailures > 0) {
            ...
            console.log("  • These are ONE problem, not two: the load can't finish while your MLS credentials\n" +
...
program
    ...
    .option("--force", "Seed even when the preflight says the feed exceeds your plan's storage.")
    .action(async (opts) => {
    ...
    if (useCheckpoint) {
        ...
        try {
            ...
            if (freshSeed) {
                ...
                for (const l of pf.lines)
                    console.log(l);
                if (pf.verdict === "exceeds" && !opts.force) {
                    console.error(`[chatrealty-sync] refusing to start a seed that cannot finish. ` +
            ...
            for (;;) {
                ...
                last = await runSyncSlice(cfg, {
                    ...
                    onPage: (p) => {
                        ...
                        console.log(`[chatrealty-sync]   … ${p.passPulled.toLocaleString()} pulled, ` +
