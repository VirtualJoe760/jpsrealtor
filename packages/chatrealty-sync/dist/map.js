...
import { RESOURCES, } from "./vendor/data-dictionary.js";
const ATTRIBUTION_PLACEHOLDER = "Information Not Available";
// Build resoName → field index from the catalog (the source of truth).
const PROPERTY_FIELDS = RESOURCES.Property.fields;
const BY_RESO_NAME = new Map(PROPERTY_FIELDS.map((f) => [f.resoName, f]));
// The set of pgColumns the catalog OWNS — anything not here is an "extra".
const CORE_RESO_NAMES = new Set(PROPERTY_FIELDS.map((f) => f.resoName));
...
function coerceBoolean(value) {
    ...
    if (["y", "yes", "true", "1", "t"].includes(s))
        return true;
    if (["n", "no", "false", "0", "f"].includes(s))
        return false;
    return null;
}
 ...
 */
export function mapResoProperty(record, opts = {}) {
    const listingKey = record.ListingKey;
    if (isBlank(listingKey))
        return null;
    const row = {};
    const extras = {};
    ...
    for (const field of PROPERTY_FIELDS) {
        if (DERIVED_COLUMNS.has(field.pgColumn))
    ...
    if (isBlank(row["mls_source"]))
        row["mls_source"] = String(record.OriginatingSystemName ?? "UNKNOWN");
    if (isBlank(row["mls_id"]))
        row["mls_id"] = String(record.OriginatingSystemID ?? record.OriginatingSystemKey ?? "UNKNOWN");
...
export function mappedPropertyColumns() {
    const cols = PROPERTY_FIELDS.filter((f) => !DERIVED_COLUMNS.has(f.pgColumn)).map((f) => f.pgColumn);
