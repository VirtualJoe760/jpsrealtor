...
const COACHELLA_VALLEY_CITIES = new Set<string>([
...
const JOSHUA_TREE_AREA_CITIES = new Set<string>([
 ...
 */
export function deriveRegion(county: string, city: string): string {
  const c = cleanCounty(county);
  if (c === "Riverside" && COACHELLA_VALLEY_CITIES.has(city)) return "Coachella Valley";
  if (c === "San Bernardino" && JOSHUA_TREE_AREA_CITIES.has(city)) return "Joshua Tree Area";
...
/** Per-city aggregate row shape returned by the city GROUP BY. */
interface CityAgg {
  city: string;
  county: string;
...
/** Per-subdivision aggregate row shape (adds the active cma block). */
interface SubAgg extends CityAgg {
  subdivision_name: string;
  active_count: number;
  active_median_price: number | null;
  active_avg_price: number | null;
  active_min_price: number | null;
  active_max_price: number | null;
  active_median_ppsf: number | null;
  active_avg_ppsf: number | null;
  active_avg_dom: number | null;
  active_median_sqft: number | null;
  active_avg_sqft: number | null;
  active_avg_beds: number | null;
  active_avg_baths: number | null;
}
...
export async function buildNeighborhoods(
  client: Queryable,
): Promise<BuildNeighborhoodsResult> {
  // ---- CITY TIER (always) ----------------------------------------------------
  const cityRes = await client.query(CITY_AGG_SQL);
  const cityRows = cityRes.rows as unknown as CityAgg[];
  ...
  const subRows = subRes.rows as unknown as SubAgg[];
...
async function upsertCity(client: Queryable, c: CityAgg): Promise<void> {
...
function cmaConfidence(activeCount: number): string {
  if (activeCount >= 20) return "high";
  if (activeCount >= 8) return "good";
...
async function refreshLocationIndex(
  client: Queryable,
  cityRows: CityAgg[],
  subRows: SubAgg[],
): Promise<number> {
  ...
  if (citySlugs.length > 0) {
    await client.query(
      `DELETE FROM location_index WHERE type = 'city' AND slug = ANY($1)`,
      [citySlugs],
    );
  }
  if (subSlugs.length > 0) {
    await client.query(
      `DELETE FROM location_index WHERE type = 'subdivision' AND slug = ANY($1)`,
      [subSlugs],
    );
  }
