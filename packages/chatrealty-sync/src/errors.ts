 ...
 */
function storageRecovery(networksAlreadySet: boolean): string[] {
  const steps = [
  ...
  if (networksAlreadySet) {
    steps.splice(
      2,
      0,
      "You already have RESO_NETWORKS set — that IS being applied to every pull, but it cannot " +
...
export function explainSyncError(
...
): SyncErrorExplanation | null {
  if (isStorageLimitError(message)) {
    return {
      code: "database_full",
      error:
        `YOUR DATABASE IS FULL. The database ChatRealty sets up for you holds about ` +
  ...
  if (isRateLimitError(message)) {
    return {
      ...
      error:
        "Your MLS feed is throttling requests right now. Your credentials are fine — the limit " +
...
export function storageLimitHelp(env: Record<string, string | undefined> = {}): string {
  const x = explainSyncError(
    "could not extend file because project size limit (512 MB) has been exceeded",
    env,
  )!;
