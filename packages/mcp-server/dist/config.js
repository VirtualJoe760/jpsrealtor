...
function loadConfig() {
    ...
    if (!apiToken) {
        throw new Error("CHATREALTY_API_TOKEN is not set. Mint a token at " +
    ...
    if (!apiToken.startsWith("crt_live_")) {
        throw new Error("CHATREALTY_API_TOKEN does not look like a ChatRealty token (expected crt_live_...). " +
