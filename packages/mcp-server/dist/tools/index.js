...
const site_tools_js_1 = require("./site_tools.js");
...
const plan_listing_carousel_js_1 = require("./plan_listing_carousel.js");
...
const IMAGES = [
    plan_listing_carousel_js_1.plan_listing_carousel,
    stage_listing_with_agent_js_1.stage_listing_with_agent,
    create_listing_cover_js_1.create_listing_cover,
    create_carousel_slide_js_1.create_carousel_slide,
];
...
exports.SERVER_INSTRUCTIONS = "TESTING PHASE: ChatRealty is currently in active testing. If a ChatRealty tool errors unexpectedly, the scaffolded site misbehaves, or the build guide contradicts what you observe, first verify it isn't a usage mistake — then file it with the `report_bug` tool (exact errors verbatim, secrets redacted) and tell the agent you did. Bug reports in this phase directly drive fixes, often same-day. When the agent wants to hand a whole session's results to ChatRealty (feedback + session log + project code), use `give_feedback` — it returns a one-time upload URL for a source-only zip (never include .env files or node_modules). " +
    "TEST DATA IS LOCALHOST-ONLY: NEVER deploy or publish a site running in test-data mode (CHATREALTY_TEST_DATA=true) to Vercel or ANY host — the sample listings are fictitious and publishing them violates MLS/IDX rules. The template's build hard-fails on deploy attempts in test mode; do not work around it. Deploying requires real data first (set CHATREALTY_API_TOKEN, remove CHATREALTY_TEST_DATA). " +
