...
exports.show_listing_board = {
    ...
    async handler(input, config) {
        ...
        const listings = await Promise.all(items.slice(0, n).map(async (it) => ({
            ...
            thumb: await (0, _media_js_1.fetchImageDataUri)(it.thumbUrl || it.primaryPhotoUrl),
        })));
        const where = filters.subdivision || filters.city || filters.near || "";
        const title = `${listings.length} active listing${listings.length === 1 ? "" : "s"}` +
            (where ? ` — ${where}` : "");
        // structuredContent → the board UI. content → the short text the model sees.
        return {
            _structuredContent: { title, listings },
            _mcpText: `Rendered an interactive board of ${listings.length} listing${listings.length === 1 ? "" : "s"}` +
                (where ? ` in ${where}` : "") +
                ". " +
                listings
                    .map((l) => `${l.address || "?"} — ${l.price ? "$" + Number(l.price).toLocaleString("en-US") : "n/a"}`)
                    .join("; "),
        };
    },
};
