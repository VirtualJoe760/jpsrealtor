...
exports.BUILD_GUIDE_PROMPTS = [
    {
        ...
        body: [
            ...
            "   - If I have one: lay out the two setup paths and recommend the second — (a) a small test fetch right here on this computer to validate my credentials and preview my feed, or (b) a VPS running the daily sync (recommended: set-and-forget, the same pattern production agents run). Tenant-database setup pairs with this and is rolling out with ChatRealty — help me get started rather than sending me away.",
            ...
            "   - EITHER WAY, offer the preview path: while the backend setup is in motion I can see the full site working NOW in TEST DATA mode — `npx create-chatrealty-site@latest my-site --test-data` scaffolds against 25 FICTITIOUS sample listings bundled with the package (no token needed). Rules you must keep: always call it test data, never present those listings as real, and never let me deploy or launch the site publicly while it runs on test data — the site's built-in banner stays until my real feed is connected. Never scrape, never invent additional listings, never build a local stand-in database beyond the bundled samples.",
    ...
    },
    {
        ...
        summary: "One command generates a complete Next.js site on your ChatRealty data — the supported starting point.",
        order: 2,
        body: [
