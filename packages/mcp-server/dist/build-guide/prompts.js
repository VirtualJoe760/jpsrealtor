...
exports.BUILD_GUIDE_PROMPTS = [
    ...
    },
    {
        ...
        body: [
            ...
            "6a-2. AND CHECK WHERE THE LINKS GO. Click a listing card, a CHAP result, a swipe-deck 'View details' and 'View all N photos' — every one must stay on THIS site (/listings/{key}). Anything that navigates to chatrealty.io is a bug: the site is the destination, not a funnel to the hub. The listing objects carry a `detailUrl` that points at the hub — it's there for chat artifacts, not for site links; use `listingHref(listingKey)` from `lib/links.ts`.",
            ...
            "   - COMPLIANCE (required before we can ever launch, so get them now): license ID number, brokerage, and team name if one exists. These must be present and easily viewable on the site — always including the footer, on every page. Put them in as soon as I give them: if my ChatRealty profile doesn't carry them yet, set AGENT_LICENSE / AGENT_BROKERAGE in .env.local so they show on the site immediately, and tell me to add them to my profile too — `set_site_live` checks the PROFILE, so an env override alone will still block go-live. If I don't have them handy, note plainly that the site cannot go live without the license number and move on.",
