...
exports.BUILD_GUIDE_PROMPTS = [
    {
        ...
        summary: "ChatRealty is bring-your-own-data — confirm YOUR tenant database is connected, or start the data-key conversation (local test fetch vs VPS sync).",
        order: 1,
        body: [
    ...
    },
    {
        ...
        summary: "Interview first, then one command wires the backend plumbing — the first site the agent sees is already THEIRS, never the neutral wireframe.",
        order: 2,
        body: [
            "Scaffold my real-estate website using ChatRealty's official generator.",
            "",
            "1. QUESTIONS COME BEFORE ANY BUILDING. In ONE message, before you run anything or show me anything, ask me for: (a) my ChatRealty API token (`crt_live_…` — minted at Settings → Integrations, the 'Website & listings' preset is exactly right; treat it like a password: server-side env only, never in client code, never committed), and (b) the DESIGN INTERVIEW — where I sell and what that place feels like (desert resort, coastal, mountain town, urban, suburbs); my market focus (luxury, investment/cash-flow, senior living, family & first-time, vacation homes); the feel I want (elegant, warm, bold, minimal, editorial) plus any brands or sites whose look I admire; brand assets (colors, logo, headshot, brokerage); typography leaning. Check `my_agent_profile` first and skip anything it already answers. Do NOT run the scaffolder until I've replied.",
            ...
            "4. APPLY MY DESIGN BEFORE THE FIRST REVEAL: using my interview answers (and the design step's craft guidance — real palette with hex values, type pairing via next/font, hero + homepage sections that fit my positioning), restyle globals.css/tailwind theme, layout header/nav/footer, the homepage, and the listing cards. THEN `npm run dev` and open http://localhost:3000 together — the first thing I see should already look like MINE.",
            "5. In that same first look, smoke-check the plumbing: listings grid shows data; map renders pins; a detail page shows 'Listed by {office} — {agent}' attribution (hard IDX display rule); the favorites heart persists; the inquiry form submits; /blog, /about, /contact, and /neighborhoods render (identity hydrates from my ChatRealty profile).",
            "5b. Enable CHAP (the on-site property chat — ChatRealty's flagship search experience): ask me for an LLM API key (Groq recommended — console.groq.com, generous free tier; any OpenAI-compatible provider works), set CHAT_API_KEY in .env.local (server-side only, never in chat or client code), restart dev. The chat bubble appears bottom-right; test it with a real search like '3 beds under 800k with a pool'. It works in test-data mode too.",
            "6. Move to the design step to present the direction you chose, refine it with me, and iterate until it's unmistakably my brand.",
            "",
    ...
    },
    {
        ...
        summary: "The backend is wired — now Claude designs a site that's actually yours: interview, proposed direction, full restyle.",
        order: 3,
        body: [
    ...
    },
    {
        ...
        summary: "Tune the scaffolded search page: filters, sorting, and card design on the real /api/skill/listings/search params.",
        order: 4,
        body: [
    ...
    },
    {
        ...
        summary: "Guest favorites plus an inquiry form that lands leads in your ChatRealty CRM — write-only from the visitor's side.",
        order: 6,
        body: [
