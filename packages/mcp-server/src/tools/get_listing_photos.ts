...
export const get_listing_photos: ToolDef = {
  ...
  description:
    "Returns ordered photo URLs for an MLS listing, plus a galleryUrl link to the full public gallery. " +
  ...
  inputSchema: {
    ...
    properties: {
      ...
      embed: {
        type: "number",
        description:
          "Number of photos to return as inline RENDERED images so they display in chat (0-12, default 0). " +
  ...
  async handler(input, config) {
    ...
    const embedN =
      typeof embed === "number" ? Math.max(0, Math.min(12, Math.floor(embed))) : 0;
