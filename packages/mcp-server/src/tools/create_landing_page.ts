...
export const create_landing_page: ToolDef = {
  name: "create_landing_page",
  description:
    "Create a landing-page DRAFT on the agent's ChatRealty site. Draft-only — the agent will review and publish from the CMS. Use this when the user says 'create a landing page', 'build a landing page on chatrealty', 'draft a page for [community/listing/event]', or similar. Pull agent context from my_agent_profile and listing/market context as needed before drafting. Returns the editUrl + previewUrl for follow-up.",
  inputSchema: {
    type: "object",
    properties: {
  ...
  async handler(input, config) {
    return await request(config, "/api/skill/landing-pages", {
      method: "POST",
      body: input,
    });
  },
