...
export const post_instagram_carousel: ToolDef = {
  ...
  inputSchema: {
    ...
    properties: {
      imageUrls: {
        ...
        maxItems: 10,
        description:
          "2–10 publicly-fetchable HTTPS image URLs (JPEG or PNG). Pull these from get_listing_photos, my_agent_profile, or any other ChatRealty tool that surfaces URLs.",
      },
      caption: {
        type: "string",
        description:
          "Post caption. Hashtags allowed (Instagram counts them toward the 2200 limit). Line breaks are preserved.",
      },
    },
  ...
  async handler(input, config) {
    const { imageUrls, caption } = input as { imageUrls: string[]; caption: string };
    return await request(config, "/api/skill/instagram/carousel", {
      method: "POST",
