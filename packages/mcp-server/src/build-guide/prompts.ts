...
export const BUILD_GUIDE_PROMPTS: readonly BuildGuidePrompt[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "6. FIRST REVEAL + smoke-check in the same look: listings grid shows data; map renders pins; 'Listed by {office} — {agent}' attribution renders on the CARDS and the detail page and the map popup (hard IDX display rule — check the cards specifically, that is where it goes missing); the favorites heart persists; the inquiry form submits (it POSTs to `/api/lead` — that is the route name, there is no `/api/inquiries`, and a judged session lost time hunting a 404 for it); the homepage market-stats strip shows a count — it walks my market cities and renders the first one that HAS active for-sale inventory, so if the whole section is missing, none of them do yet and the answer is more seeding, not a broken endpoint (the dev server says exactly that in its log); /blog, /about, /contact render under the RIGHT NAME (see the identity rule in 0b — read the header and <title>, don't assume); /neighborhoods lists the markets I actually serve, not a scatter of cities from elsewhere in the feed (it is built from my service areas — set AGENT_SERVICE_AREAS in .env.local if my profile has none).",
